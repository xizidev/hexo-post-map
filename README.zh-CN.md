...
详情地点只显示静默图钉，不打开地图服务商的弹窗。`route` 中的地点按顺序编号，并始终以直线示意段连接，不表示可导航道路。
...
地图初始视野包含全部文章代表位置。每张缩略图通过短线连接到可见坐标圆点，不会遮住位置锚点。文章根据当前缩放和屏幕距离自动合并或展开；聚合圆会随文章数量分为小、中、大三档，同时点击后仍通过缩放展开。如果放大不能分离文章，或已达到最大缩放，则打开与缩略图预览、**全部文章**入口共用的有界文章面板（移动端为底部抽屉）。面板限制最大高度，文章列表只在面板内部滚动。点击预览图片或标题进入文章。地图不可用时仍保留按时间排序的文章列表。
